...
const SingleCast = ({ castDetails, actedIn, getMovie }) => {
  ...
  return (
    <motion.main variants={pageVariant} initial="initial" animate="visible" exit="exit" className="single-cast">
      <section className="single-cast-section">
        ...
        {actedIn && actedIn.length > 0 ? (
          <section className="single-cast-acted-in">
            <h3>Acted in</h3>
            <ul className="single-cast-acted-in-movies">
              {actedIn.map(({ id, title, poster_path }) => (
                <li key={id} className="similar-movie">
                  <Link onClick={() => getMovie(id)} to={`/movies/${title.toLowerCase().replace(/ /g, "-")}`}>
                    <FallbackImage
                      alt={title}
