...
const Genres = ({ genreListMovies, currentGenre, PaginatedItems, getMovie }) => {
  useEffect(() => {
    window.scrollTo({ top: 0, left: 0, behavior: "smooth" });
  }, []);

  return (
    <motion.main variants={pageVariant} initial="initial" animate="visible" exit="exit" className="genres-route">
      <h2>{currentGenre}</h2>
          ...
          <motion.li key={movie.id} className="homepage-movie" whileHover={{ scale: 1.1 }}>
            ...
            <section className="homepage-movie-meta">
              ...
              <section className="homepage-movie-meta-title">
                ...
                <h3 className="all-movies-rating">{movie.vote_average.toFixed(2)}</h3>
              </section>
            </section>
          </motion.li>
        ))}
      </ul>
      {genreListMovies.length === 0 ? (
        <p>Sorting by genres is currently unavailable. Please check the API or contact the website owner.</p>
      ) : (
        PaginatedItems
      )}
    </motion.main>
